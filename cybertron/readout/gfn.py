...
class LoRAModulatedGFNIteration(nn.Module):
    ...
    def __call__(self, s_i, z_ij, x_i, s_mask, sigma, rg):
        """
        ## Args:
        ...
        for k in range(self.n_node_interactions):
            ...
            s_i = self.layer_norm_s[k](s_i) # post_layer_norm: normalize node rep
...
class GFNReadout(nn.Module):
    ...
    def setup(self):
        ...
        GFNIter = nn.checkpoint(LoRAModulatedGFNIteration) if REMAT_FLAG \
                    else LoRAModulatedGFNIteration
    ...
    def __call__(self, s_i, z_ij, x_i, s_mask, noise, rg):
        x_i, noise, rg = jax.tree_map(lambda x:jnp.array(x, dtype=self._safedtype), 
                                     (x_i, noise, rg))
        
        s_i, z_ij = self.gfn_iter_0(s_i, z_ij, x_i, s_mask, noise, rg)
