...
def linear_vanilla(
    ...
    w = [
        get_parameter(
            ...
            ins.path_shape,
            ins.weight_std,
            jnp.float32, #### liuyh modified here
        )
        for ins in linear.instructions
    ]
    
    _dtype = jnp.bfloat16 if BF16_FLAG else jnp.float32
    # w, input = promote_dtype(w, input, dtype=_dtype) 
    w = promote_dtype(*w, dtype=_dtype) ##### liuyh modified here
    input = input.astype(_dtype) ##### liuyh modified here
