...
from e3nn_jax._src.batchnorm.bn import batch_norm
    ...
    def __call__(
        ...
        if not self.instance:
            ra_mean = self.variable(
                "batch_stats",
                "mean",
                lambda s: jnp.zeros(s, jnp.float32), #### liuyh modified here
                (num_scalars,),
            )
            ra_var = self.variable(
                "batch_stats", "var", lambda s: jnp.ones(s, jnp.float32), (num_irreps,) #### liuyh modified here
            )
        else:
            ra_mean = None
            ra_var = None
        ...
        if self.affine:
            weights = self.param("weights", lambda _: jnp.ones((num_irreps,), jnp.float32))
            biases = self.param("biases", lambda _: jnp.zeros((num_irreps,), jnp.float32))
        else:
            weights = None
            biases = None
